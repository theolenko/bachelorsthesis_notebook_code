...
# Sachsen_PDFExtractor.py — Extrahiert Texte aus den PDFs, basierend auf Sachsen_Meta.json
...
# wichtig: lege multi_column.py ins gleiche Verzeichnis https://github.com/pymupdf/PyMuPDF-Utilities/blob/master/text-extraction/multi_column.py
...
IMPORTANT_FILTERS = {
    ...
    "biogas", "erdgas", "leag", "kernnetz", "panter", "mibrag"
}
...
# Dateien
META_FILE   = os.path.join(os.path.dirname(__file__), 'Sachsen_Meta.json')
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), 'Sachsen_Data.json')
...
def choose_filters_to_use(filters: list[str], total_pages: int) -> list[str]:
    """
    Wenn PDF >300 Seiten: nur IMPORTANT_FILTERS, falls welche existieren, sonst alle.
    Sonst alle.
    """
    if total_pages > 300:
...
def extract_text(url: str, filters: list[str]) -> str:
    ...
    multicolumn via multi_column.column_boxes.
    """
    try:
...
def parse_links(element_id: str) -> list[tuple[str,str]]:
    """
    Teilt "Label1:URL1 | Label2:URL2" in [(Label1, URL1), (Label2, URL2), …].
    """
...
def main():
    ...
    # 3) Durch alle Elemente iterieren, bereits vorhandene überspringen
    for ent in entries:
        ...
        # für jeden Link in der gruppierten ElementID
        texts = []
    ...
    logging.info("PDF-Extraktion Sachsen abgeschlossen.")
